...
def eval_tracks(
    ...
    for path_dataset in tqdm(path_datasets):
    ...
    print("Compute ",category," metrics")
    summary = mh.compute(
        acc_c,
        metrics=[
            "num_frames",
    ...
    result_list = [mota,motp_c,motp_o,motp_i,idf1,most_track,most_lost,num_fp,num_miss,num_switch,num_frag]
    out_string = (
        # f"{fn} {num_frames} {mota:.2f} {motp_c:.2f} {motp_o:.2f} {motp_i:.2f} {idf1:.2f} {most_track:.2f} "
        f"{mota:.2f} {motp_c:.2f} {motp_o:.2f} {motp_i:.2f} {idf1:.2f} {most_track:.2f} "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--d_max", type=float, default=100, required=False)
    parser.add_argument(
        "--diffatt",
        type=str,
        default=None,
    ...
    out_filename = f"{tk_basename}_evaluation.csv"

    logger.info("output file name = %s", out_filename)
    
    out_file_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)),'Argo_Tracking_Evaluation')
    ...
    with open(out_file_path, "w",newline='') as out_file:
        ...
        print("Start to evaluate VEHICLE tracker performance")
        result_C = eval_tracks(
        ...
        print("Get VEHICLE metrics")
        print("\nStart to evaluate PEDESTRIAN tracker performance")
        result_P = eval_tracks(
            args.path_tracker_output,
            args.path_dataset,
            args.d_min,
            args.d_max,
        ...
        print("Get PEDESTRIAN metrics")
        writer = csv.writer(out_file)
        writer.writerow(['C:MOTA','P:MOTA','C:MOTPD','P:MOTPD','C:MOTPO','P:MOTPO','C:MOTPI','P:MOTPI','C:IDF1','P:IDF1','C:MT','P:MT','C:ML','P:ML','C:FP','P:FP','C:FN','P:FN','C:SW','P:SW','C:FRG','P:FRG'])
        row = []
        for i in range(11):
            row.append(f"{result_C[i]:.2f}")
            row.append(f"{result_P[i]:.2f}")
        writer.writerow(row)
