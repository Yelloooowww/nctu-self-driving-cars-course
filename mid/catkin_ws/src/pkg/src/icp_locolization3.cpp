...
//   if (pcl::io::loadPCDFile<pcl::PointXYZI> ("/home/bory/sdc/localization/map/nuscenes_maps/map_1700_1000.pcd", *load_map) == -1)
  {
    PCL_ERROR ("Couldn't read file map.pcd \n");
    exit(0);
  }

  cout<<"read map finish"<<endl;
  cout<<"original map : "<<load_map->points.size()<<endl;
  ...
  cout<<"sor_map filter : "<<load_map->points.size()<<endl;
  ...
  int init_x= 1716.40466004;
  int init_y= 1014.50164193;
  int init_z= -0.365639155011;
...
void icp_locolization::cb_lidar_scan(const sensor_msgs::PointCloud2 &msg)
{
  ...
  sor_lidar.setLeafSize (0.1f, 0.1f, 0.1f);
  sor_lidar.filter (*lidar_sor);
  pcl::fromPCLPointCloud2(*lidar_sor, *bag_cloud);
  cout<<"sor_lidar filter: "<<bag_cloud->points.size()<<endl;
